...
//Printing the final transition table
...
//Driver Code for Accepting Inputs
int main(){
	int n,alpha;
	cout<<"----------------------- NFA to DFA -------------------------"<<endl<<endl;
	cout<<"Enter the total number of states in NFA : ";
	cin>>n;
	cout<<"Enter the number of input symbols (in alphabets) : ";
	cin>>alpha;
	vector<vector<vector<int> > > table;
	
	//Providing input for each individual state
	for(int i=0;i<n;i++){
		...
		//Providing input for Epsilon State
		vector<int> t;
		cout<<"Enter no. of output states for input e : ";
		cin>>yn;
		cout<<"Enter output states :"<<endl;
		for(int k=0;k<yn;k++){
			cin>>y;
	...
	//Printing the Transition Table of DFA
